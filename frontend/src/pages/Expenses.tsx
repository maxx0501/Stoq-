...
export const Expenses = ({ onNavigate, onLogout, user, storeName, setUser }: any) => {
  ...
  return (
    <Layout active="expenses" onNavigate={onNavigate} onLogout={onLogout} user={user} storeName={storeName} setUser={setUser}>
            <div className="max-w-6xl mx-auto">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6 md:mb-8">
                    <div>
                        <h1 className="text-2xl md:text-3xl font-black text-slate-800">Despesas</h1>
                        <p className="text-slate-500 text-sm">Controle suas contas fixas e variáveis.</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 mb-6 md:mb-8">
                    <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center relative overflow-hidden">
                        ...
                        <div>
                            ...
                            <h2 className="text-2xl md:text-3xl font-black text-slate-800">{formatMoney(totalPending)}</h2>
                        </div>
                    ...
                    <div className="bg-white p-4 md:p-6 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center relative overflow-hidden">
                        ...
                        <div className="w-10 h-10 md:w-12 md:h-12 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center">
                            <CheckCircle size={20}/>
                        </div>
                    </div>
                </div>
                        ...
                        {filteredExpenses.map((expense: any) => (
                            <div key={expense.id} className="p-4 space-y-2">
                                <div className="flex justify-between items-center">
                                    <span className="font-bold text-slate-700 text-sm truncate flex-1 mr-2">{expense.description}</span>
                                    <button onClick={() => togglePaid(expense.id)} className={`flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg shrink-0 ${expense.paid ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'}`}>
                                ...
                                <div className="flex justify-between items-center text-xs">
                                    <div className="flex items-center gap-2">
                                        <span className="bg-slate-100 px-2 py-0.5 rounded text-[10px] font-bold text-slate-500">{expense.category}</span>
                                        <span className={`flex items-center gap-1 ${new Date(expense.dueDate).getTime() < new Date().getTime() && !expense.paid ? 'text-red-500' : 'text-slate-500'}`}>
                                            <Calendar size={12}/> {formatDate(expense.dueDate)}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="font-black text-slate-800">{formatMoney(Number(expense.value))}</span>
        ...
        {showModal && (
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in duration-200">
                <div className="bg-white rounded-t-3xl md:rounded-3xl p-6 md:p-8 max-w-sm w-full shadow-2xl max-h-[90vh] overflow-y-auto">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <button onClick={() => setShowModal(false)} className="p-2 hover:bg-slate-100 rounded-full transition"><X size={20} className="text-slate-400"/></button>
                    </div>
